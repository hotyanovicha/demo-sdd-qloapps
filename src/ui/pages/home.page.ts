...
export class HomePage extends BasePage {
  ...
  private readonly calendarDay = (day: string) =>
    this.page.locator('.day.toMonth.valid').filter({ hasText: new RegExp(`^${day}$`) }).first().describe(`Calendar day: ${day}`);
  ...
  async fillSearchForm(hotelName: string, checkInDay: string, checkOutDay: string): Promise<void> {
    ...
    await this.hotelOption(hotelName).click();
    await this.datepickerTrigger.click();
    await this.calendarDay(checkInDay).click();
    await this.calendarDay(checkOutDay).click();
  }
